...
class Solution {
 public:
  int stoneGameII(std::vector<int>& piles) {
    ...
    std::vector<int> suffixSums = piles;
    for (int i = n - 2; i >= 0; --i) {
    ...
    for (int i = n - 1; i >= 0; --i) {
      for (int m = n - 1; m >= 1; --m) {
        ...
        for (int x = 1; x <= 2 * m && i + x < n; ++x) {
          int opponent = dp[i + x][std::max(x, m)];
          dp[i][m] = std::max(dp[i][m], suffixSums[i] - opponent);
        }
      }
    }
  ...
  int stoneGameIITopDown(const std::vector<int>& piles) {
    int n = piles.size();
    std::vector<int> suffix = piles;
    for (int i = n - 2; i >= 0; --i) {
      suffix[i] += suffix[i + 1];
    }
  ...
  int maxStones(const std::vector<int>& suffix,
                int idx,
                int m,
    ...
    for (int x = 1; x <= 2 * m; ++x) {
      nextPlayer = std::min(nextPlayer,
                            maxStones(suffix, idx + x, std::max(x, m), memo));
    }
