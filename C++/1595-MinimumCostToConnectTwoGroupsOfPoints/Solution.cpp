...
class Solution {
 public:
  int connectTwoGroups(std::vector<std::vector<int>>& cost) {
    // size1 * size2 cost matrix, where cost[i][j] represent the cost of
    // connecting node i in group1 to node j in group2.
    // Each node in each group must be connected to at least 1 node in the other
    // group.
    // Constraint: size1 >= size2.
    //
    // Essentially Min Cost Bipartite Matching.
    // Kuhn-Munkres Hungarian Algorithm.
    // WARNING: not so simple. Hungarian algorithm finds a perfect matching,
    // i.e., one-to-one. Whereas in some cases, a one-to-many connection is
    // desired.
    // Need to preprocess.
    std::vector<std::vector<int>> mat = transpose(cost);
    const int m = mat.size();
    const int n = mat[0].size();

    int sumMin = 0;
    std::vector<int> lmin(m, INT_MAX);
    std::vector<int> rmin(n, INT_MAX);
    for (int l = 0; l < m; ++l) {
      for (int r = 0; r < n; ++r) {
        lmin[l] = std::min(lmin[l], mat[l][r]);
      }
      sumMin += lmin[l];
    }
    ...
    for (int l = 0; l < m; ++l) {
      for (int r = 0; r < n; ++r) {
        mat[l][r] = std::min(mat[l][r] - lmin[l] - rmin[r], 0);
      }
    }

    int mc = Hungarian<false, true>(mat);
    return sumMin + mc;
  }
