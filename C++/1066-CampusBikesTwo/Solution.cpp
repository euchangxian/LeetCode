...
class Solution {
 public:
  int assignBikes(std::vector<std::vector<int>>& workers,
    ...
    // workers[i] = (x,y)
    // bikes[j]   = (x,y), indicating points on a 2D grid.
    // Assign one unique bike to each worker so that the sum of the
    // Manhattan Distance between each worker and their assigned bike is
    // minimised.
    // Return the minimum possible sum of MD between each worker and their
    // assigned bikes.
    // MD(p1, p2) = |p1.x - p2.x| + |p1.y - p2.y|
    //
    // Weighted MCBM, where the costMatrix is the MD.
    int n = workers.size();
    int m = bikes.size();

    auto calcMD = [](const auto& p1, const auto& p2) {
      return std::abs(p1[0] - p2[0]) + std::abs(p1[1] - p2[1]);
    };

    // 1-indexed
    std::vector<std::vector<int>> costMatrix(n + 1, std::vector<int>(m + 1));
    for (int i = 1; i <= n; ++i) {
      for (int j = 1; j <= m; ++j) {
        costMatrix[i][j] = calcMD(workers[i - 1], bikes[j - 1]);
      }
    }

    auto minCost = Hungarian<false, false, int>(costMatrix);
    return minCost;
  }
};
